...
class MemoryCache @JvmOverloads internal constructor(
    ...
    init {
        CoroutineScope(fetchParentJob + Executors.newVirtualThreadPerTaskExecutor().asCoroutineDispatcher()).launch {
            val fetchReqs = mutableMapOf<PathSlice, MutableSet<CompletableDeferred<ArrowBuf>>>()

            try {
                for (req in fetchCh) {
                    when (req) {
                        is FetchReq -> {
                            val (pathSlice, fetch, res) = req
                        ...
                        is FetchDone -> {
                            val (pathSlice, localPath, onEvict) = req
                                        ...
                                        cacheAl.wrapForeignAllocation(
                                            object : ForeignAllocation(memSeg.byteSize(), memSeg.address()) {
                                                override fun release0() {
                                                    arena.close()
                                                    onEvict?.close()
                                                    openSlices.computeIfPresent(req.pathSlice) { _, buf ->
