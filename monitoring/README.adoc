...
To enable OpenTelemetry tracing in your XTDB node, configure it under the `otel` section:
...
* `endpoint` (string) – OTLP gRPC endpoint for sending traces. Default: `"http://localhost:4317"`
* `service-name` (string) – Service name for traces. Default: `"xtdb"` (or `"xtdb-<node-id>"` if node-id is set)
