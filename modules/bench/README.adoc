...
You can run a benchmark on the cloud using the "run-bench.sh" script:
[source,bash]
----
./cloud/run-bench.sh <aws|azure|google-cloud> <benchmark-type> <opts...>
----

TPC-H::
[source,bash]
----
./cloud/run-bench.sh azure tpch --scale-factor 0.01
----
Readings::
[source,bash]
----
./cloud/run-bench.sh azure readings --devices 10000 --readings 10000
----

At the beginning of each run, we clear the contents of the cloud's storage bucket and the previous helm release of the benchmark/kafka to fully clear the data. To prevent this behavior, you can pass the `--no-cleanup` flag to the script.

==== Running Auctionmark

Typically, we run Auctionmark in two stages - an initial load-phase and then an OLTP phase running with multiple nodes/pods. To run using run-bench:

Load Phase::
[source,bash]
----
./cloud/run-bench.sh azure auctionmark --scale-factor 0.1 --onlyLoad true
----

OLTP Phase::
[source,bash]
----
./cloud/run-bench.sh azure auctionmark --threads 8 --duration PT10M --no-cleanup
----

=== Cleaup the Infra

To fully clearup the storage, helm deployments and infrastructure on the cloud, you can run the "delete-infra.sh" script:
